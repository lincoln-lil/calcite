...
public class SqlToRelConverterTest extends SqlToRelTestBase {
  ...
  @Test public void testTemporalWithLateral() {
    final String sql = "select stream * from "
        + "orders join lateral products_temporal for system_time as of orders.rowtime "
  ...
  @Test public void testCollectionTableWithLateral4() {
    sql("select * from "
     + "(select * from dept, lateral table(ramp(deptno))), lateral table(ramp(deptno))").ok();
  }

  @Test public void testCollectionTableWithLateral5() {
    sql("select * from dept, lateral table(DEDUP(dept.deptno, dept.name)), "
      + "lateral table(ramp(deptno))").ok();
  }
