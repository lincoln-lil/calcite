...
public class TypeCoercionTest extends TypeCoercionTestCase {
  ...
  private SqlToRelTestBase.Tester tester2;

  // type category.
  private ImmutableList<RelDataType> integeralTypes;
  private ImmutableList<RelDataType> fractionalTypes;
  private ImmutableList<RelDataType> numericTypes;
  private ImmutableList<RelDataType> atomicTypes;
  private ImmutableList<RelDataType> complexTypes;
  ...
  private RelDataType dateSecondIntervalType;
  ...
  private RelDataType varchar10Type;
  ...
  private void initializeSingleTypes() {
    ...
    dateSecondIntervalType = dataTypeFactory.createSqlIntervalType(
    ...
    varchar10Type = dataTypeFactory.createSqlType(SqlTypeName.VARCHAR, 10);
  ...
  private void initializeCategoryTypes() {
    ...
    integeralTypes = builder.build();
    // FRACTIONAL
    ImmutableList.Builder<RelDataType> builder1 = ImmutableList.builder();
    for (SqlTypeName typeName : SqlTypeName.FRACTIONAL_TYPES) {
      builder.add(dataTypeFactory.createSqlType(typeName));
    }
    fractionalTypes = builder1.build();
    // NUMERIC
    ImmutableList.Builder<RelDataType> builder2 = ImmutableList.builder();
    for (SqlTypeName typeName : SqlTypeName.NUMERIC_TYPES) {
      builder2.add(dataTypeFactory.createSqlType(typeName));
    }
    ...
    List ll = ImmutableList.of(Pair.of("a1", varbinaryType), Pair.of("a2", intType));
    builder4.add(dataTypeFactory.createStructType(ll));
    complexTypes = builder4.build();
    //ALL
  ...
  private RelDataType recordType(List pairs) {
  ...
  // we need this because some types has the same type family, i.e. TIMESTAMP and
  ...
  @Test public void testGetTightestCommonType() {
    ...
    // we use RelDataTypeFactory#leastRestrictive to find the common type, it's not symmetric but
    // it's ok cause precision does not comes lower.
  ...
  @Test public void testSetOperations() {
    ...
    checkPlanEquals(
        "select t1c, t1g, t1b, t1f from t1 "
            + "union select t2a, t2g, t2e, t2d from t2 "
            + "union select t1a, t1g, t1e, t1f from t1 "
            + "union select t2a, t2g, t2b, t2f from t2",
        "LogicalUnion(all=[false])\n"
            + "  LogicalUnion(all=[false])\n"
            + "    LogicalUnion(all=[false])\n"
            + "      LogicalProject(T1C=[CAST($2):VARCHAR CHARACTER SET "
            + "\"ISO-8859-1\" COLLATE \"ISO-8859-1$en_US$primary\" NOT NULL], "
            + "T1G=[$6], T1B=[$1], T1F=[$5])\n"
            + "        LogicalTableScan(table=[[CATALOG, SALES, T1]])\n"
            + "      LogicalProject(T2A=[$0], T2G=[$6], T2E=[$4], T2D=[$3])\n"
            + "        LogicalTableScan(table=[[CATALOG, SALES, T2]])\n"
            + "    LogicalProject(T1A=[$0], T1G=[$6], T1E=[$4], T1F=[$5])\n"
            + "      LogicalTableScan(table=[[CATALOG, SALES, T1]])\n"
            + "  LogicalProject(T2A=[$0], T2G=[$6], T2B=[$1], T2F=[$5])\n"
            + "    LogicalTableScan(table=[[CATALOG, SALES, T2]])\n");
    sql("select t1c, t1g, t1b, t1f from t1 "
        + "union select t2a, t2g, t2e, t2d from t2 "
        + "union select t1a, t1g, t1e, t1f from t1 "
        + "union select t2a, t2g, t2b, t2f from t2")
        .type("RecordType(VARCHAR NOT NULL T1C, "
            + "DECIMAL(19, 0) NOT NULL T1G, "
            + "FLOAT NOT NULL T1B, "
            + "DOUBLE NOT NULL T1F) NOT NULL");

    // intersect
    sql("select t1c, t1g, t1b, t1f from t1 "
        + "intersect select t2a, t2g, t2e, t2d from t2 ")
        .type("RecordType(VARCHAR NOT NULL T1C, "
            + "DECIMAL(19, 0) NOT NULL T1G, "
            + "FLOAT NOT NULL T1B, "
            + "DOUBLE NOT NULL T1F) NOT NULL");
    // except
    sql("select t1c, t1g, t1b, t1f from t1 "
        + "except select t2a, t2g, t2e, t2d from t2 ")
        .type("RecordType(VARCHAR NOT NULL T1C, "
            + "DECIMAL(19, 0) NOT NULL T1G, "
            + "FLOAT NOT NULL T1B, "
            + "DOUBLE NOT NULL T1F) NOT NULL");
  ...
  @Test public void testArithmeticExpressionsWithStrings() {
    ...
    sql("select 1+'2', 2-'3', 2*'3', 2/'3', MOD(4,'3') "
        ...
        .type("RecordType(DOUBLE NOT NULL EXPR$0, "
            + "DOUBLE NOT NULL EXPR$1, "
            + "DOUBLE NOT NULL EXPR$2, "
            + "DOUBLE NOT NULL EXPR$3, "
            + "DECIMAL(19, 0) NOT NULL EXPR$4) NOT NULL");
    checkExp("select abs(t1a) from t1");
    checkExp("select sum(t1a) from t1");
    checkExp("select avg(t1a) from t1");
    tester.setFor(SqlStdOperatorTable.STDDEV_POP);
    tester.setFor(SqlStdOperatorTable.STDDEV_SAMP);
    checkExp("select STDDEV_POP(t1a) from t1");
    checkExp("select STDDEV_SAMP(t1a) from t1");
    checkExp("select -(t1a) from t1");
    checkExp("select +(t1a) from t1");
    tester.setFor(SqlStdOperatorTable.VAR_POP);
    tester.setFor(SqlStdOperatorTable.VAR_SAMP);
    checkExp("select VAR_POP(t1a) from t1");
    checkExp("select VAR_SAMP(t1a) from t1");
  }
  ...
  @Test public void testBinaryComparable() {
    ...
    checkPlanEquals(
        ...
        "LogicalProject(EXPR$0=[true])\n"
            + "  LogicalValues(tuples=[[{ true }]])\n");
    checkPlanEquals(
        "select t1i > t1h from t1",
        "LogicalProject(EXPR$0=[>(CAST($8):TIMESTAMP(0) NOT NULL, $7)])\n"
            + "  LogicalTableScan(table=[[CATALOG, SALES, T1]])\n");
  }

  /** Test cases for {@link TypeCoercion#inOperationConversion}. */
  @Test public void testIn() {
    checkPlanEquals(
        "select 1 in ('1', '2', '3') from (values true)",
        "LogicalProject(EXPR$0=[true])\n"
            + "  LogicalValues(tuples=[[{ true }]])\n");
    checkPlanEquals(
        ...
        "LogicalProject(EXPR$0=[CAST(CASE(=($1, 0), false, IS NOT NULL($7), true, "
            ...
            + "          LogicalValues(tuples=[[{ true, true }]])\n");
    checkPlanEquals(
        "select (1, 2) in (('1', '2'), ('3', '4')) from (values true)",
        "LogicalProject(EXPR$0=[true])\n"
            + "  LogicalValues(tuples=[[{ true }]])\n");
  }
  ...
  @Test public void testBooleanEquality() {
    // REVIEW chenyuzhao 2018-05-16: Now we do not support cast between numeric <-> boolean,
    // we should support this later on, we add cast in the plan so other systems
    // using Calcite can rewrite Cast operator then do the cast work.
    ...
    checkPlanEquals(
        ...
        "LogicalProject(EXPR$0=[true])\n"
            + "  LogicalValues(tuples=[[{ true }]])\n");
    checkPlanEquals(
        "select 1.0=true from (values true)",
        "LogicalProject(EXPR$0=[true])\n"
            + "  LogicalValues(tuples=[[{ true }]])\n");
    checkPlanEquals(
        "select 0.0=true from (values true)",
        "LogicalProject(EXPR$0=[false])\n"
            + "  LogicalValues(tuples=[[{ true }]])\n");
    checkPlanEquals(
        "select 1.23=t1k from t1",
        "LogicalProject(EXPR$0=[=(false, $10)])\n"
            + "  LogicalTableScan(table=[[CATALOG, SALES, T1]])\n");
    // int boolean
    checkPlanEquals(
        "select t1b=t1k from t1",
        "LogicalProject(EXPR$0=[=($1, CASE($10, 1, 0))])\n"
            + "  LogicalTableScan(table=[[CATALOG, SALES, T1]])\n");
    checkPlanEquals(
        "select 10000000000=true from (values true)",
        "LogicalProject(EXPR$0=[false])\n"
            + "  LogicalValues(tuples=[[{ true }]])\n");
  }

  /** Test case for `when clause` and `COALESCE` FUNC. */
  @Test public void testCaseWhen() {
    // coalesce
    // double int float
    sql("select COALESCE(t1f, t1c, t1e) from t1")
        .type("RecordType(DOUBLE NOT NULL EXPR$0) NOT NULL");
    // bigint int decimal
    sql("select COALESCE(t1d, t1c, t1g) from t1")
        .type("RecordType(DECIMAL(19, 0) NOT NULL EXPR$0) NOT NULL");
    // null int
    sql("select COALESCE(null, t1c) from t1")
        .type("RecordType(INTEGER EXPR$0) NOT NULL");
    // timestamp varchar
    sql("select COALESCE(t1a, t1h) from t1")
        .type("RecordType(VARCHAR NOT NULL EXPR$0) NOT NULL");
    // null float int
    sql("select COALESCE(null, t1e, t1c) from t1")
        .type("RecordType(FLOAT EXPR$0) NOT NULL");
    // null int decimal double
    sql("select COALESCE(null, t1c, t1g, t1f) from t1")
        .type("RecordType(DOUBLE EXPR$0) NOT NULL");
    // null float double varchar
    sql("select COALESCE(null, t1e, t1f, t1a) from t1")
        .type("RecordType(VARCHAR NOT NULL EXPR$0) NOT NULL");
    // timestamp int varchar
    sql("select COALESCE(t1h, t1c, t1a) from t1")
        .type("RecordType(TIMESTAMP(0) NOT NULL EXPR$0) NOT NULL");

    // case when
    // smallint int char
    sql("select case when 1 > 0 then t2b when 2 > 3 then t2c else t2a end from t2")
        .type("RecordType(VARCHAR NOT NULL EXPR$0) NOT NULL");
    // boolean int char
    sql("select case when 1 > 0 then t2k when 2 > 3 then t2c else t2a end from t2")
        .type("RecordType(VARCHAR NOT NULL EXPR$0) NOT NULL");
    // float decimal
    sql("select case when 1 > 0 then t2e else t2g end from t2")
        .type("RecordType(DOUBLE NOT NULL EXPR$0) NOT NULL");
    // bigint decimal
    sql("select case when 1 > 0 then t2d else t2g end from t2")
        .type("RecordType(DECIMAL(19, 0) NOT NULL EXPR$0) NOT NULL");
    // plan check
    checkPlanEquals("select case when 1 > 0 then t2d else t2g end from t2",
        "LogicalProject(EXPR$0=[CASE(>(1, 0), CAST($3):DECIMAL(19, 0) NOT NULL, $6)])\n"
            + "  LogicalTableScan(table=[[CATALOG, SALES, T2]])\n");
  }
  ...
  @Test public void testImplicitCasts() {
    ...
    shouldCast(checkedType10, SqlTypeFamily.DECIMAL,
        SqlTypeUtil.getDefaultPrecisionScaleDecimal(dataTypeFactory));
  ...
  /** Test case for {@link AbstractTypeCoercion#implicitCast}. */
  @Test  public void testImplicitCastExpression() {
    // concat
    checkExpType("'ab'||'cde'", "CHAR(5) NOT NULL");
    checkExpType("null||'cde'", "VARCHAR");
    checkExpType("1||'234'", "VARCHAR NOT NULL");
    checkExpFails("select ^'a'||t1j^ from t1",
        "(?s).*Cannot apply.*");
    // smallint int double
    checkExpType("select t1b||t1c||t1f from t1", "VARCHAR");
    // boolean float smallint
    checkExpType("select t1k||t1e||t1b from t1", "VARCHAR");
    // decimal
    checkExpType("select t1g||t1a from t1", "VARCHAR");
    // date timestamp
    checkExpType("select t1h||t1i from t1", "VARCHAR");

    checkPlanEquals(
        "select 1||'a' from (values true)",
        "LogicalProject(EXPR$0=[||(CAST(1):VARCHAR CHARACTER SET \"ISO-8859-1\""
            + " COLLATE \"ISO-8859-1$en_US$primary\" NOT NULL, 'a')])\n"
            + "  LogicalValues(tuples=[[{ true }]])\n");
  }

  /** Test cases for
   * {@link org.apache.calcite.sql.validate.implicit.TypeCoercionImpl#divisionToFractional}. */
  @Test public void testDivideToDecimal() {
    // cast operands of / always to decimal.
    sql("select 4/3 from (values true)")
        .type("RecordType(DOUBLE NOT NULL EXPR$0) NOT NULL");
    sql("select -4.0/3 from (values true)")
        .type("RecordType(DECIMAL(13, 12) NOT NULL EXPR$0) NOT NULL");
    sql("select 4/3.0 from (values true)")
        .type("RecordType(DECIMAL(17, 6) NOT NULL EXPR$0) NOT NULL");
    // float/int
    sql("select t1e/3 from t1")
        .type("RecordType(DOUBLE NOT NULL EXPR$0) NOT NULL");
    // null
    sql("select t1e/null from t1")
        .type("RecordType(DOUBLE EXPR$0) NOT NULL");
  }

  @Test public void testStarImplicitTypeCoercion() {
    checkPlanEquals(
        "select * from (values(1, '3')) union select * from (values('2', 4))",
        "LogicalUnion(all=[false])\n"
            + "  LogicalProject(EXPR$0=[CAST($0):VARCHAR CHARACTER SET \"ISO-8859-1\""
            + " COLLATE \"ISO-8859-1$en_US$primary\" NOT NULL], EXPR$1=[$1])\n"
            + "    LogicalValues(tuples=[[{ 1, '3' }]])\n"
            + "  LogicalProject(EXPR$0=[$0], EXPR$1=[CAST($1):VARCHAR CHARACTER SET \"ISO-8859-1\""
  ...
  private void checkPlanEquals(String sql2, String expectedPlan) {
    RelNode rel = tester2.convertSqlToRel(sql2).project();

    assertTrue(rel != null);
    assertValid(rel);

    String actual = RelOptUtil.toString(rel);
    assertThat(actual, is(expectedPlan));
  }
  ...
  /** A catalog reader with table t1 and t2 which have all the needed test data types. */
  private  class TCatalogReader extends MockCatalogReader {

    TCatalogReader(RelDataTypeFactory typeFactory) {
    ...
    public MockCatalogReader init() {
      ...
      // Register "T1" table.
      final MockTable t1 =
          MockTable.create(this, tSchema, "T1", false, 7, null, null);
      t1.addColumn("t1a", varchar20Type, true);
      t1.addColumn("t1b", smallintType);
      t1.addColumn("t1c", intType);
      t1.addColumn("t1d", bigintType);
      t1.addColumn("t1e", floatType);
      t1.addColumn("t1f", doubleType);
      t1.addColumn("t1g", decimalType);
      t1.addColumn("t1h", timestampType);
      t1.addColumn("t1i", dateType);
      t1.addColumn("t1j", binaryType);
      t1.addColumn("t1k", booleanType);
      registerTable(t1);

      final MockTable t2 =
          MockTable.create(this, tSchema, "T2", false, 7, null, null);
      t2.addColumn("t2a", varchar20Type, true);
      t2.addColumn("t2b", smallintType);
      t2.addColumn("t2c", intType);
      t2.addColumn("t2d", bigintType);
      t2.addColumn("t2e", floatType);
      t2.addColumn("t2f", doubleType);
      t2.addColumn("t2g", decimalType);
      t2.addColumn("t2h", timestampType);
      t2.addColumn("t2i", dateType);
      t2.addColumn("t2j", binaryType);
      t2.addColumn("t2k", booleanType);
      registerTable(t2);
      return this;
    }
  }
