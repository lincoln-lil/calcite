...
public class SqlInOperator extends SqlBinaryOperator {
  ...
  public RelDataType deriveType(
    ...
    if (right instanceof SqlNodeList) {
      ...
      if (null == rightType && validator.getEnableTypeCoercion()) {
    ...
    // Do type coercion first.
    if (callBinding.getValidator().getEnableTypeCoercion()) {
      boolean changed = callBinding.getValidator().getTypeCoercion()
          .inOperationConversion(callBinding);
      if (changed) {
        // update the node data type if we did type coercion.
